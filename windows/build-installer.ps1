...
& $Compiler "/DAppVersion=$version" "/DFrameworkUrl=$($manifest.framework.url)" "/DFrameworkHash=$($manifest.framework.sha256)" "/DVcUrl=$($manifest.vc.url)" "/DVcHash=$($manifest.vc.sha256)" "$PSScriptRoot/installer/eScratch.iss"
