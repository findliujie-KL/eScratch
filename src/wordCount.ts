...
const individualRanges: readonly (readonly [number, number])[] = [
  [0x1100, 0x11FF],
  [0x2010, 0x2010],
  [0x2016, 0x2016],
  [0x2025, 0x2025],
...
function isIndividual(c: number): boolean {
  for (const [start, end] of individualRanges) {
...
export function countWords(text: string): number {
  let count = 0, inRun = false, canAttachVariation = false
  for (const character of text) {
    const c = character.codePointAt(0)!
    if (c === 0x200B || c === 0xFEFF) continue
    if ((c >= 0x09 && c <= 0x0D) || [0x20, 0xA0, 0x2005, 0x2013, 0x2014, 0x2022, 0x3000].includes(c)) {
      inRun = false; canAttachVariation = false; continue
    }
    if (canAttachVariation && ((c >= 0xFE00 && c <= 0xFE0F) || (c >= 0xE0100 && c <= 0xE01EF))) continue
